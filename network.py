...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        # embedding layer
        #这个完了是几维。debug看一下，是不是这里embedding的时候出错了，只保留了句子本身的信息，所以不同的句子可以预测
        # word_embedding: 16661 * 100
        # pos_embedding: 5 * 123
        #values: n*70*100, n*70*5, n*70*5,  input_forward: n*70*110
        ...
        with tf.variable_scope('GRU_FORWARD') as scope:
            #这个地方有问题，如果句子的长度没有70的怎么办，之前做了填充，都用的blank，这个是个问题，把长度改小一点。
        ...
        #ouputs_forward: 70 * n * 200, 这里n不是total_num， 为什么后面reshape是了？
        ...
        # t1 = tf.reshape(tf.tanh(output_h), [total_num * num_steps, gru_size])
        # t2 = tf.reshape(tf.matmul(t1, attention_w),[total_num, num_steps])
        # t3 = tf.reshape(tf.nn.softmax(t2),[total_num, 1, num_steps])
        # t4 = tf.reshape(tf.matmul(t3, output_h), [total_num, gru_size])
