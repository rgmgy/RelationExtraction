...
#两个entity可以有多种关系，而且这个最后没有处理，按照同一个样本有多个标签进行了训练，改进
#一个疾病和多个症状往往出现在一句话里面，所以位置embedding的时候改进一下
...
def init():
    ...
    vec = []
    word2id = {}
    f = io.open('./origin_data/vec.txt', encoding='utf-8')
    content = f.readline()
    ...
    relation2id = {}
    f = io.open('./origin_data/relationRETRainDataFinal', 'r', encoding='utf-8')
    ...
    #train_sen的格式为什么是这样的，不应该是一个pair一个最小的基本
    ...
    f = io.open('./origin_data/RETRainDataFinal', 'r', encoding='utf-8')
    ...
    f = io.open('./origin_data/RETestData.txt', 'r', encoding='utf-8')
    ...
    f = io.open('./data/Healthtrain_q&a.txt', 'w', encoding='utf-8')
    temp = 0
    for i in train_sen:
    ...
    f =  io.open('./data/Healthtest_q&a.txt', 'w', encoding='utf-8')
    temp = 0
    for i in test_sen:
    ...
    np.save('./data/Healthtrain_x.npy', train_x)
    np.save('./data/Healthtrain_y.npy', train_y)
    np.save('./data/Healthtestall_x.npy', test_x)
    np.save('./data/Healthtestall_y.npy', test_y)
...
def seperate():
    ...
    x_train = np.load('./data/Healthtrain_x.npy')
    ...
    #最后保持的训练数据格式，看一下保持的每个样本什么样
    train_word = np.array(train_word)
    train_pos1 = np.array(train_pos1)
    train_pos2 = np.array(train_pos2)
    np.save('./data/Healthtrain_word.npy', train_word)
    np.save('./data/Healthtrain_pos1.npy', train_pos1)
    np.save('./data/Healthtrain_pos2.npy', train_pos2)

    print('seperating test all data')
    x_test = np.load('./data/Healthtestall_x.npy')
    test_word = []
    test_pos1 = []
    test_pos2 = []
    ...
    np.save('./data/Healthtestall_word.npy', test_word)
    np.save('./data/Healthtestall_pos1.npy', test_pos1)
    np.save('./data/Healthtestall_pos2.npy', test_pos2)



# get answer metric for PR curve evaluation
def getans():
    test_y = np.load('./data/Healthtestall_y.npy')
    eval_y = []
    for i in test_y:
        eval_y.append(i[1:])
...
def get_metadata():
    fwrite = io.open('./data/metadata.tsv', 'w', encoding='utf-8')
    f = io.open('./origin_data/vec.txt', encoding='utf-8')
    f.readline()
    while True:
